...
public class Rates {
     ...
     */
     public boolean isValidDates() {
        if (beginDate == null ||
                endDate == null) {
            details = "Invalid.  One of the dates was not defined";
            return false;
        } else if (beginDate.getYear() != endDate.getYear()) {
                details = "Invalid.  The begin and end date must be within the same year.";
                return false;
        } else if (beginDate.equals(endDate)) {
                details = "Invalid.  The begin and end date must not be the same date";
                return false;
            ...
            if (!beginDate.after(endDate)) {
                if ((!beginDate.before(seasonStartMonth, seasonStartDay)) &&
                    ...
                    details = "Invalid.  Begin or end date was out of season";
                    return false;
                }
            } else {
                details = "Invalid. End date was before begin date";
                return false;
            }
        } else {
            details = "Invalid.  One of the dates was not a valid day";
            return false;
        }
   ...
   //     System.out.println("baseRate = " + baseRate + ", premiumRate = " + premiumRate + ", rate = " + rate);
    ...
    public boolean setDuration(int days) {
        ...
        if (!valid) {
            ...
            //setEndDate(endDate); //added to method
            synched = false; 
            return true;
        }
    }
